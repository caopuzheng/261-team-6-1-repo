...
# MAGIC __`MIDS w261: Machine Learning at Scale | UC Berkeley School of Information | Spring 2024`__
# MAGIC
# MAGIC > Updated: 03/07/2024
# MAGIC
...
# MAGIC
# MAGIC * This is a relatively [type] matrix. If self loops are allowed, then we could have a total of [number] edges. Otherwise, if self loops are not allowed, we could have [number] edges. Out of a total possible edges, this graph only has 9 edges. For sparse graphs, their adjacency list representation will be [efficiency] than their adjacency matrix (memory wise). 
# MAGIC
# MAGIC
...
# MAGIC
# MAGIC * This is a [graph_type] graph. Because we see the edge ('A', 'B') in the graph, but there is no corresponding ('B', 'A'). For directed graphs, the adjacency matrix will be [symmetric]. Whereas for undirected graphs, it will be [symmetric]. 
# MAGIC
...
# MAGIC
# MAGIC * The Markov Property is the property of [property]. In the context of Page Rank, this is the assumption that [assumption].
# MAGIC
...
# MAGIC
# MAGIC * The n states are the [states]. Therefore, the size of the transition matrix will be [size].
# MAGIC
# MAGIC > __d)__ Numeric Answer: _____
# MAGIC
# MAGIC > __f)__ Numeric Answer: _____
# MAGIC
...
# MAGIC * The raw data is in the format of [type].
# MAGIC * The first value of each line represents [value].
# MAGIC * The second part of each line represents [part].
# MAGIC
# MAGIC > __b)__ Highlight the correct answer (Add ** before and after your selected choice):
...
# MAGIC > __b)__ Numeric Answer: _____
# MAGIC
# MAGIC > __c)__ Highlight the correct answer (Add ** before and after your selected choice):
# MAGIC
# MAGIC * '7858931'
...
# MAGIC
# MAGIC > __c)__ Numeric Answer: _____
# MAGIC
# MAGIC > __e)__ Highlight the correct answer (Add ** before and after your selected choice):
# MAGIC
# MAGIC * 1184351
